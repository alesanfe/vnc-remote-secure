...
## Context
When nginx is enabled as a reverse proxy, internal services (VNC, noVNC, ttyd,
health server) do not need to be accessible from external networks. Binding them
to 0.0.0.0 exposes them directly, bypassing nginx's SSL, rate limiting, and
access controls.
