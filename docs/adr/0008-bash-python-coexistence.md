# ADR-0008: Coexistence of Bash and Python implementations
...
3. **Windows PowerShell** (`native/windows/`): Native Windows commands
...
|----------|----------------------|----------------|
| Linux | `src/rpi-vnc-remote.sh` | Bash + Python services |
| Windows | `VncRemote.ps1` / `vnc-remote` CLI | PowerShell + Python services |
...
- **Windows**: `native/windows/` PowerShell commands manage UltraVNC,
...
- **Mitigation**: This ADR documents the boundary. The `core/constants.py`
  module is the single source of truth for defaults. The `config/defaults/`
