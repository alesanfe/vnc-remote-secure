...
- **Rate Limiting** - DDoS protection in nginx
- **SSL Auto-renewal** - Certificates renewed automatically

### 🚀 Advanced Features
- **Nginx Reverse Proxy** - Single-port access with SSL
- **Monitoring Stack** - Prometheus + Grafana (optional)
- **Health Monitoring** - CPU, memory, disk checks
- **Session Recording** - Terminal and VNC recording (optional)
- **User Management UI** - Web interface for administration
...
- **Domain Name**: For SSL/TLS (DuckDNS, No-IP, etc.)
...
TTYD_PASSWD=your_password ./src/rpi-vnc-remote.sh
```
...
# Configure SSL
cp .env.example .env
nano .env  # Set DUCK_DOMAIN and EMAIL
...
# Access services
curl http://localhost/vnc/
curl http://localhost/terminal/
```
...
│   ├── quick-start.md       # Quick start (5 minutes)
│   ├── detailed-setup.md    # Complete setup
