...
generate_ssl_certificates() {
    ...
    if sudo netstat -tulpn 2>/dev/null | grep -q ':80 '; then
        log "yellow" "Port 80 is in use, stopping nginx temporarily..."
        sudo systemctl stop nginx 2>/dev/null || true
        # Wait a moment for nginx to stop
        sleep 2
        # Check again
        if sudo netstat -tulpn 2>/dev/null | grep -q ':80 '; then
            log "red" "Port 80 is still in use after stopping nginx"
            log "yellow" "Please manually stop the service using port 80"
            return 1
        else
            log "green" "Port 80 is now available"
        fi
    fi
    ...
    if sudo certbot certonly --standalone --preferred-challenges http \
        ...
        log "yellow" "Continuing without SSL encryption"
        return 1
    fi
}
...
fix_ssl_permissions() {
    ...
    if ! id "$TEMP_USER" &>/dev/null; then
        log "yellow" "User $TEMP_USER does not exist yet, skipping permission fix"
        return 0
    fi
...
setup_ssl() {
    ...
    if generate_ssl_certificates; then
        ...
        sudo cp "$DUCK_DIR/fullchain.pem" "$SSL_CERT"
        sudo cp "$DUCK_DIR/privkey.pem" "$SSL_KEY"
        log "blue" "SSL certificates saved to:"
        log "blue" "  Certificate: $SSL_CERT"
        log "blue" "  Private Key: $SSL_KEY"
        fix_ssl_permissions
        export DISABLE_SSL=false
