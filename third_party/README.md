...
runtime by `launch.sh` into the `novnc/` directory (which is gitignored).

If you need to vendor noVNC, use a git submodule pointing to a specific
commit:

```bash
git submodule add https://github.com/novnc/noVNC.git third_party/novnc
cd third_party/novnc
